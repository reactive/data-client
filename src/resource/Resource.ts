...
import { schemas, SchemaOne, SchemaMany } from './normal';
...
export default abstract class Resource {
  ...
  static detailShape<T extends typeof Resource>(
    this: T,
  ): ReadShape<SchemaOne<AbstractInstanceType<T>>> {
    const self = this;
    const getFetchKey = (params: Readonly<object>) => {
      return 'GET ' + this.url(params);
    };
    const schema: SchemaOne<AbstractInstanceType<T>> = this.getEntitySchema();
  ...
  static listShape<T extends typeof Resource>(
    this: T,
  ): ReadShape<SchemaMany<AbstractInstanceType<T>>> {
    const self = this;
    const getFetchKey = (params: Readonly<Record<string, string>>) => {
      return 'GET ' + this.listUrl(params);
    };
    const schema: SchemaMany<AbstractInstanceType<T>> = [
  ...
  static createShape<T extends typeof Resource>(
    ...
    SchemaOne<AbstractInstanceType<T>>,
    Readonly<object>,
