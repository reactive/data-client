...
const TestComponent = ({
  ...
  return (
    <DataProvider gcPolicy={new GCPolicy({ intervalMS, expiryMultiplier })}>
      <MockResolver
        fixtures={[
          {
...
describe('Integration Garbage Collection Web', () => {
  ...
  it('should count properly with useSuspense', async () => {
    ...
    await testSuspenseFlow();
    jest.useRealTimers();
  });

  it('should expiryMultiplier GC option should work', async () => {
    jest.useFakeTimers();

    await testSuspenseFlow(
      (ArticleResource.getList.dataExpiryLength ?? 60000) * 4,
      { intervalMS: GC_INTERVAL, expiryMultiplier: 4 },
    );
...
async function testSuspenseFlow(
  expiryTime = (ArticleResource.getList.dataExpiryLength ?? 60000) * 2,
  gcOptions: GCOptions = { intervalMS: GC_INTERVAL },
) {
  render(<TestComponent {...gcOptions} />);
  ...
  // Jest should not expire yet
  act(() => {
    jest.advanceTimersByTime(expiryTime / 2 + GC_INTERVAL);
  });
  await testGCSweep();
  ...
  // Now it's enough time to expire
  act(() => {
    jest.advanceTimersByTime(expiryTime / 2 + GC_INTERVAL);
  });
  await testGCSweep();
