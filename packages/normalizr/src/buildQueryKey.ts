import type { Schema, SchemaSimple, QuerySnapshot } from './interface.js';
 ...
 */
export default function buildQueryKey(snapshot: QuerySnapshot) {
  return function queryKey<S extends Schema>(
    ...
    // schema classes
    if (canQuery(schema)) {
      return schema.queryKey(args as any[], queryKey, snapshot);
    }

    // plain case
    if (typeof schema === 'object' && schema) {
      const method = Array.isArray(schema) ? arrayQuery : objectQuery;
      return method(schema, args, queryKey, snapshot);
    }
