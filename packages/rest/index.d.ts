...
declare type EntityMap<T = any> = Record<string, EntityInterface<T>>;
...
/** @deprecated */
declare type SchemaDetail<T> = EntityInterface<T> | {
    [K: string]: any;
} | SchemaClass$1;
/** @deprecated */
declare type SchemaList<T> = EntityInterface<T>[] | {
    [K: string]: any;
...
declare type OnlyOptional<S extends string> = S extends `${infer K}?` ? K : never;
declare type OnlyRequired<S extends string> = S extends `${string}?` ? never : S;
/** Computes the union of keys for a path string */
declare type PathKeys<S extends string> = string extends S ? string : S extends `${infer A}\\:${infer B}` ? PathKeys<A> | PathKeys<B> : S extends `${infer A}\\?${infer B}` ? PathKeys<A> | PathKeys<B> : PathSplits<S>;
declare type PathSplits<S extends string> = S extends `${string}:${infer K}/${infer R}` ? PathSplits<`:${K}`> | PathSplits<R> : S extends `${string}:${infer K}:${infer R}` ? PathSplits<`:${K}`> | PathSplits<`:${R}`> : S extends `${string}:${infer K}` ? K : never;
/** Parameters for a given path */
declare type PathArgs<S extends string> = PathKeys<S> extends never ? unknown : KeysToArgs<PathKeys<S>>;
...
declare type PathArgsAndSearch<S extends string> = OnlyRequired<PathKeys<S>> extends never ? Record<string, number | string | boolean> | undefined : {
    [K in PathKeys<S> as OnlyRequired<K>]: string | number;
} & Record<string, number | string>;
/** Removes the last :token */
declare type ShortenPath<S extends string> = string extends S ? string : S extends `${infer B}:${infer R}` ? TrimColon<`${B}:${ShortenPath<R>}`> : '';
declare type TrimColon<S extends string> = string extends S ? string : S extends `${infer R}:` ? R : S;
