...
**Escalation valve**: after two distinct failed hypotheses on the same problem — or when another attempt would cost more than a consult — stop. This applies equally to environment, build, and tooling failures, not just the code you are writing: before diagnosing such a failure in depth, run the cheapest experiment that discriminates "caused by my changes" from "pre-existing" (e.g. stash your diff and re-run); if pre-existing and outside the task's scope, document it and move on rather than fixing the environment. Package the failure evidence (error output, what you tried, why each attempt failed) and consult `design-advisor`, or `principal-advisor` if the failure is correctness-deep. If a principal consult already exists, resume it with the failure evidence rather than opening a fresh one — it has the context already and a follow-up costs a fraction of a new consult. Never repeat an approach without new evidence; cheap-model spin is the hidden cost that erases the price advantage.
