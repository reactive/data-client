...
- **Detection decision**: keep duck-typing (`__ownerID` own-prop for Map, `_map.__ownerID` for Record) — no new peer dependency; code now lives only on the `/imm` path. Supported range: immutable v4–v5 (both expose these internals; devDep pins 5.x, B3 adds the v4 CI cell).
