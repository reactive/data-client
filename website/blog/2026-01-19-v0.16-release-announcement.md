...
- [path-to-regexp v8](/blog/2026/01/19/v0.16-release-announcement#path-to-regexp-v8) - [RestEndpoint.path](/rest/api/RestEndpoint#path) syntax updated: `/:optional?` → `{/:optional}`, `/:repeat+` → `/*repeat`
...
| `/:repeat+` | `/*repeat` | One-or-more repeating |
| `/:repeat*` | `{/*repeat}` | Zero-or-more repeating |
| `/:with-dash` | `/:"with-dash"` | Quoted parameter name |
| `/(regex)` | `N/A` | Custom regex removed |

Characters `()[]{}*:;,!@` must now be escaped with `\\` when used as literals.

See [path-to-regexp errors](https://github.com/pillarjs/path-to-regexp#errors) for detailed explanations
of each change. Or use AI-assisted migration:
