...
- [useFetch() stable promise](/blog/2026/01/19/v0.16-release-announcement#usefetch-stable-promise) - Always returns a promise; check `.resolved` instead of truthiness
...
[useFetch()](/docs/api/useFetch) now always returns a promise with a `.resolved` property, even when data is already cached. The same promise reference is preserved across re-renders, making it compatible with [React.use()](https://react.dev/reference/react/use) and [useLoading()](/docs/api/useLoading).
...
The returned promise works with `React.use()` for Suspense:
...
And with [useLoading()](/docs/api/useLoading) for non-Suspense loading states:
