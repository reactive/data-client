...
- [useFetch() stable promise](/blog/2026/01/19/v0.16-release-announcement#usefetch-stable-promise) - Always returns a promise, works with `React.use()` and [useLoading()](/docs/api/useLoading)
...
[useFetch()](/docs/api/useFetch) now always returns a promise with a `.resolved` property, even when data is already cached. The same promise reference is preserved across re-renders, making it compatible with `React.use()` and easy to compose with [useLoading()](/docs/api/useLoading).

### React.use()

Since `useFetch()` always returns a promise, it can be passed directly to [React.use()](https://react.dev/reference/react/use) to suspend until data is available:
...
The returned promise works naturally with [useLoading()](/docs/api/useLoading) for non-Suspense loading states:

```tsx
import { useFetch, useLoading } from '@data-client/react';
...
Use `promise.resolved` to check whether the fetch has completed:

```tsx
function PostPage({ id }: { id: number }) {
  const promise = useFetch(PostResource.get, { id });
  // Previously: if (promise) → fetch was triggered
  // Now: if (!promise.resolved) → fetch is in-flight
  if (!promise.resolved) return <Spinner />;
  // ...
}
```
