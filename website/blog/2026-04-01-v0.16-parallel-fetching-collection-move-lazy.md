---
title: 'v0.16: Parallel Fetching, Collection.move, Lazy'
description: Parallel data loading with useFetch(), Collection.move for reordering across lists, and Lazy for deferred relationship denormalization
image: /img/social/0.16-card.png
authors: [ntucker]
...
- [Parallel data loading with useFetch()](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#parallel-data-loading) - Fetch multiple endpoints concurrently with `use(useFetch())`, avoiding sequential waterfalls
- [Collection.move](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#collection-move) - Move entities between [Collections](/rest/api/Collection) with a single operation
- [Collection.moveWith()](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#collection-move) - Customize move behavior (e.g., prepend instead of append)
- [Lazy](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#lazy) - Deferred relationship denormalization for performance and memoization isolation

**Other Improvements:**

- [Direct schema imports](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#direct-schema-imports) - Import schema classes directly without the `schema` namespace
- [Denormalization depth limit](/blog/2026/04/01/v0.16-parallel-fetching-collection-move-lazy#denormalization-depth-limit) - Prevent stack overflow in large bidirectional entity graphs; configurable via [`Entity.maxEntityDepth`](/rest/api/Entity#maxEntityDepth) ([#3822](https://github.com/reactive/data-client/issues/3822))
...
- [renderDataHook()](/docs/api/renderDataHook) automatic cleanup after each test — no manual `cleanup()` calls needed
...
Upgrade with an automated [codemod](/codemods/v0.16.js) for all breaking changes:
...
## Denormalization depth limit & Lazy

When schemas have bidirectional relationships across entity types
(e.g., `Department → Building → Department → ...`), denormalization traverses every unique
...
Two complementary features address this: a **depth limit** as a safety net, and
[**Lazy**](/rest/api/Lazy) for precise, per-field control over which relationships are resolved eagerly.
...
This prevents crashes, but is a blunt instrument — it cuts off *all* deep paths,
including legitimate non-cyclic ones. For fine-grained control, use [Lazy](#lazy).

### Lazy

[Lazy](/rest/api/Lazy) wraps a schema to skip eager denormalization of specific relationship fields.
During parent entity denormalization, the field retains its raw normalized value (primary keys).
The relationship can then be resolved on demand via [useQuery](/docs/api/useQuery) using the `.query` accessor.
...
needed, and provides **memoization isolation** — changes to lazy entities don't invalidate
the parent's denormalized form.

See [Lazy documentation](/rest/api/Lazy) for full usage details and
[#3828](https://github.com/reactive/data-client/discussions/3828) for design discussion.
...
Characters including `{}()[]+?!:*\\` must be escaped with `\\` when used as **literals** in the path pattern.
These characters are [reserved by path-to-regexp](https://github.com/pillarjs/path-to-regexp#express--4x) and
unescaped usage throws an [Unexpected token error](https://github.com/pillarjs/path-to-regexp#unexpected--or-).
A literal query string in the pattern still needs an escaped `?`, e.g. `'/resource\\?per_page=50'`.

When query segments use **named parameters**, prefer explicit groups instead of a single escaped `?`:
...
All schema classes are available as direct exports: [Union](/rest/api/Union), [Invalidate](/rest/api/Invalidate), [Collection](/rest/api/Collection), [Query](/rest/api/Query), [Values](/rest/api/Values), [All](/rest/api/All), and [Lazy](/rest/api/Lazy). The `schema` namespace export remains available for backward compatibility.
