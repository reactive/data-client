...
- [Schema.normalize() takes a delegate](/blog/2026/04/24/v0.18-scalar-typed-downloads#normalize-delegate) - `normalize(input, parent, key, args, visit, delegate)` → `normalize(input, parent, key, delegate)`. Affects custom [Schema](/docs/api/Schema) implementations only.
...
The breaking changes in this release affect only **custom [Schema](/docs/api/Schema) implementations**.
...
[`INormalizeDelegate`](/docs/api/Schema), matching the new denormalize delegate shape.
...
#### Optional: consolidate Collection definitions
